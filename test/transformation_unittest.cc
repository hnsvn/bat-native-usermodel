...
#include "huhi/vendor/bat-native-usermodel/src/transformation.h"
...
TEST_F(TransformationTest, ToLowerTest) {
  ...
  EXPECT_TRUE(DataType::TextData == lower_datapoint.type);
...
TEST_F(TransformationTest, HashingTest) {
  ...
  EXPECT_EQ(DataType::SparseVector, vector_data.type);
...
TEST_F(TransformationTest, ChainingTest) {
  ...
  EXPECT_EQ(DataType::SparseVector, last_point.type);
